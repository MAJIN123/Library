...
class User extends CI_Controller 
{
    ...
    public function ajax_user_list()
	{
        ...
        foreach ($list as $user) 
        {
            ...
            $row[]='<a class="btn btn-sm btn-info" href="javascript:void()" title="详细信息" onclick="detailed_info('."'".$user->student_number."'".')"><i class="glyphicon glyphicon-pushpin"></i>详细信息</a>
            <a class="btn btn-sm btn-primary" href="javascript:void()" title="修改" onclick="edit_user('."'".$user->student_number."'".')"><i class="glyphicon glyphicon-edit"></i> 修改</a>';
            //add html for action
    ...
    {
        if($this->input->post('permission_id')==null)
        {
            echo json_encode(array("error"=>'permission_id为空!'));
            exit();
        }
        $data=array(
            'student_number'=>$this->input->post('student_number'),
